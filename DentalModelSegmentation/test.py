...
if __name__ == '__main__':
    ...
    file_path = os.path.join(os.path.dirname(ARGS.output), 'results_lower.csv')
    
    for i, file in enumerate(test_files):
        ls=glob.glob(os.path.join(file, '*.obj'))
        print('ls', ls)
        if len(ls)!=0:
            ...
            header = ['name', 'TSA', 'TIR', 'TLA', 'Score', 'TSA_avg', 'TIR_avg', 'TLA_avg', 'teeth_count']
            file_is_empty = not os.path.exists(file_path) or os.path.getsize(file_path) == 0
            with open(file_path, 'a+', newline="") as f:
                writer_csv = csv.writer(f)
                if file_is_empty:
                    writer_csv.writerow(header)
                writer_csv.writerow([file, tsa, tir, tla, score, total_tsa, total_tir, total_tla, teeth_count])
