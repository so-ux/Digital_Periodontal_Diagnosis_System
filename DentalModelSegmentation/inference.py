...
def inference(input, output, model_path='../model', rotation=True, debug_vis=False):
    ...
    with torch.no_grad():
        ...
        if rotation:
            ...
            if debug_vis:
                np.savetxt(
                    f'/hpc/data/home/bme/v-tanmh/dental/txt/{os.path.basename(output)}.rot.xyz',
        ...
        if debug_vis:
            np.savetxt(
                f'/hpc/data/home/bme/v-tanmh/dental/txt/{os.path.basename(output)}.pn.txt',
        ...
        if debug_vis:
            np.savetxt(f'/hpc/data/home/bme/v-tanmh/dental/txt/{os.path.basename(output)}.ct.xyz',
                       kp_reg)
        ...
        if len(kp_reg) == 0:
            ...
            if debug_vis:
                np.savetxt(f'/hpc/data/home/bme/v-tanmh/dental/txt/{os.path.basename(output)}.ct.txt',
            ...
            patches_tensor_16k = infer_set.get_patches_tensor(True)  # torch.Size([15, 4096, 8])
            ...
            for i in range(math.ceil(len(patches_tensor_32k) / batch_size)):
                if i * batch_size == len(patches_tensor_32k) - 1:
                    ...
                    pred_seg1 = model_refine_pct_16k(
                        patches_tensor_16k[i * batch_size:(i + 1) * batch_size].transpose(2, 1))
                    pred_seg1 = torch.argmax(pred_seg1, dim=1)

                    pred_seg2 = model_refine_pct_32k(
                        patches_tensor_32k[i * batch_size:(i + 1) * batch_size].transpose(2, 1))
                    pred_seg2 = torch.argmax(pred_seg2, dim=1)
            ...
            # torch.Size([15, 16384, 7])
            # torch.Size([15, 4096, 7])
            ...
            if debug_vis:
                #########################################################
                # 保存预测的Patch1
                out_dir = f'/hpc/data/home/bme/v-tanmh/dental/txt/{os.path.basename(output)}_patches'
        ...
        mesh.export(os.path.join(output, os.path.basename(input).split('.')[0] + '.off'))
        np.savetxt(os.path.join(output, os.path.basename(input).split('.')[0] + '_label.txt'), labels, fmt='%.00f')
...
if __name__ == '__main__':
    ...
    for patient in os.listdir(args.input):
        patient_path = os.path.join(args.input, patient)
        output_path = os.path.join(patient_path, 'oral scan seg')

        # Step1: IOS model segmentation
        for name in ['_LowerJawScan', '_UpperJawScan']:
            ios_path = os.path.join(patient_path, patient + name + '.off')
