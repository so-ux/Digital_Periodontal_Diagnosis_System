...
def model_func(data, labels, centroids):
    ...
    seg_int_tensor[torch.argmax(seg, dim=1) > 0.5] = 1
    seg_int_tensor = seg_int_tensor.to(dtype=torch.int32).cuda()

    acc = mean_intersection_over_union(seg_int_tensor, seg_mask)
    # loss_cls = nn.CrossEntropyLoss()(cls, labels)

    # cls_ = torch.argmax(cls, dim=1)

    # loss_cd = 0

    # for bi in range(data.shape[0]):
    #     for tid in torch.unique(labels[bi]):
    #         if tid > 0:
    #             gt_tooth = data[bi, labels[bi] == tid, 0:3]
    #             pred_tooth = data[bi, cls_[bi] == tid, 0:3]
    #
    #             if pred_tooth.shape[0] > 0:
    #                 # loss_cd += torch.mean(chamfer_distance_without_batch(gt_tooth, pred_tooth)[0])
    #                 # loss_cd += torch.mean(chamfer_distance_without_batch(pred_tooth, gt_tooth)[0])
    #                 loss_cd += nn.SmoothL1Loss()(torch.mean(gt_tooth, dim=0), torch.mean(pred_tooth, dim=0))
    #
    #     for tid in torch.unique(cls_[bi]):
    #         if tid > 0:
    #             gt_tooth = data[bi, labels[bi] == tid, 0:3]
    #             pred_tooth = data[bi, cls_[bi] == tid, 0:3]
    #
    #             if gt_tooth.shape[0] > 0:
    #                 # loss_cd += torch.mean(chamfer_distance_without_batch(gt_tooth, pred_tooth)[0])
    #                 # loss_cd += torch.mean(chamfer_distance_without_batch(pred_tooth, gt_tooth)[0])
    #                 loss_cd += nn.SmoothL1Loss()(torch.mean(gt_tooth, dim=0), torch.mean(pred_tooth, dim=0))

    # return (cls, seg), (loss_seg, loss_cls, loss_cd)
...
if __name__ == '__main__':
    ...
    experiment_dir = '/home/zsj/PycharmProjects/miccai-3d-teeth-seg/output/experiment_raw_pn2_0809/'
    writer = TensorboardUtils(experiment_dir).writer

    data_dir = '/home/zsj/PycharmProjects/miccai-3d-teeth-seg/data/'
    train_set = TeethTrainingDataset(os.path.join(data_dir, 'train.list'), train=True)
    test_set = TeethTrainingDataset(os.path.join(data_dir, 'test.list'), train=False)
    ...
    for i in range(0, 1000):
        ...
        for batch_i, batch_data in enumerate(tqdm.tqdm(train_loader)):
            optimizer.zero_grad()
            data, labels, centroids, real_size = batch_data

            seg, loss, acc = model_func(data, labels, centroids)
            ...
            for batch_i, batch_data in enumerate(tqdm.tqdm(test_loader)):
                ...
                if i % 5 == 0:
                    for ii in range(len(data)):
                        ...
                        nd_seg = np.argmax(seg[ii].data.cpu().numpy(), axis=0)
                        # nd_cls = nd_cls.transpose(1, 0)
                        # nd_cls = np.argmax(nd_cls, axis=1)
                        # color = np.zeros((nd_data.shape[0], 4))
                        color_seg = np.zeros((nd_data.shape[0], 4))
                        for pt in range(nd_data.shape[0]):
                            # color[pt, 0:3] = colors.get_color(nd_cls[pt], True)
                            # color[pt, 3] = 1
                            color_seg[pt, 0:3] = np.repeat(np.array([nd_seg[pt]]), 3)
                            color_seg[pt, 3] = 1

                        # np.savetxt(
                        #     '/home/zsj/PycharmProjects/miccai-3d-teeth-seg/temp/validate/test_pn_ct/{}_{}.txt'.format(count,
                        #                                                                                         ii),
                        #     np.concatenate((nd_data, color), axis=1))
                        np.savetxt(
