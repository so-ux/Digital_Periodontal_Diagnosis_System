...
class ContrastHead(nn.Module):
    ...
    def __init__(self, head_cfg, config):
        ...
        assert head_cfg.sample in ['cnt', 'glb', 'sub', 'subspatial', 'pts', 'label',
                                   'vote'], f'not support sample = {head_cfg.sample}'
        # self.sample_func = getattr(self, f'sample_{head_cfg.sample}') if 'sample' in head_cfg and head_cfg.sample else self.sample_label
        self.main_contrast = getattr(self,
                                     f'{head_cfg.main}_contrast') if 'main' in head_cfg and head_cfg.main else self.point_contrast
        ...
        if 'project' in head_cfg and head_cfg.project:
            self.project = nn.ModuleDict({
                f'{n}{i}': MLPbyOps(head_cfg.project, config.base_fdim * 2 ** i, d_out=config.base_fdim) for n, i in
                self.stages
            })
    ...
    def sample_label(self, n, i, stage_list, target):
        ...
        labels = get_subscene_label(n, i, stage_list, target, self.nstride,
                                    self.num_classes)  # (m, ncls) - distribution / onehot
    ...
    def dist_kl(self, features, neighbor_feature, normalized, normalized_neighbor):
        ...
        dist = F.kl_div(neighbor_feature, features, reduction='none',
                        log_target=log_target)  # [m, nsample, d] - kl(pred, gt) to calculate kl = gt * [ log(gt) - log(pred) ]
    ...
    def point_contrast(self, n, i, stage_list, target):
        ...
        nsample = self.nsample[i]
        labels = get_subscene_label(n, i, stage_list, target, self.nstride,
                                    self.config.num_classes)  # (m, ncls) - distribution / onehot
        neighbor_idx, _ = pointops.knnquery(nsample, p, p, o, o)  # (m, nsample)

        # exclude self-loop
        nsample = self.nsample[
                      i] - 1  # nsample -= 1 can only be used if nsample is py-number - results in decreasing number if is tensor, e.g. 4,3,2,1,...
        neighbor_idx = neighbor_idx[..., 1:].contiguous()
        m = neighbor_idx.shape[0]
