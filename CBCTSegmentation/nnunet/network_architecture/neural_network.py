...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        print('run _internal_predict_3D_3Dconv_tiled')
        ...
        if regions_class_order is None:
            ...
            if len(class_probabilities_here)==5:
                for c in range(len(class_probabilities_here)):
                    if c==4:
                        predicted_segmentation[class_probabilities_here[c] > 0.7] = c
                    else:
                        predicted_segmentation[class_probabilities_here[c] > 0.5] = c
            else:
            ...
            for i, c in enumerate(regions_class_order):
                if len(regions_class_order)==5 and c==4:
                    predicted_segmentation[class_probabilities_here[i] > 0.7] = c
                else:
    ...
    def _internal_predict_2D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
        ...
        """
        print('run _internal_predict_2D_2Dconv_tiled')
    ...
    def _internal_predict_3D_3Dconv(self, x: np.ndarray, min_size: Tuple[int, ...], do_mirroring: bool,
        ...
        This one does fully convolutional inference. No sliding window
        """
        print('run _internal_predict_3D_3Dconv')
        assert len(x.shape) == 4, "x must be (c, x, y, z)"
    ...
    def _internal_maybe_mirror_and_pred_2D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        #   everything in here takes place on the GPU. If x and mult are not yet on GPU this will be taken care of here
        #   we now return a cuda tensor! Not numpy array!
        print('run _internal_maybe_mirror_and_pred_2D')
    ...
    def _internal_predict_3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
        ...
        print('run _internal_predict_3D_2Dconv')
        if all_in_gpu:
            raise NotImplementedError
    ...
    def predict_3D_pseudo3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
                                   ...
                                   pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                   verbose: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        print('run predict_3D_pseudo3D_2Dconv')
        if all_in_gpu:
            raise NotImplementedError
        assert len(x.shape) == 4, "data must be c, x, y, z"
    ...
    def _internal_predict_3D_2Dconv_tiled(self, x: np.ndarray, patch_size: Tuple[int, int], do_mirroring: bool,
                                          ...
                                          verbose: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        print('run _internal_predict_3D_2Dconv_tiled')
        if all_in_gpu:
            raise NotImplementedError
