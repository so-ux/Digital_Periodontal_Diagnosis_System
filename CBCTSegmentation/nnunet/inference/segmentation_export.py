...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
                                         ...
                                         interpolation_order_z: int = 0,remove=0, verbose: bool = True):
    """
    ...
    print('region_class_order',region_class_order)
    ...
    #12.15
    if remove!=0:
        for label in range(1, len(np.unique(seg_old_size_postprocessed))):
            print('remove bone!--------------------')
            label_mask = (seg_old_size_postprocessed == label).astype(int)
    
            labeled_array, num_features = ndimage.label(label_mask)
    
            for i in range(1, num_features + 1):
                component_size = (labeled_array == i).sum()
                if component_size < remove:
                    label_mask[labeled_array == i] = 0
    
            seg_old_size_postprocessed[seg_old_size_postprocessed == label] = 0
            seg_old_size_postprocessed[label_mask == 1] = label
...
def save_segmentation_nifti(segmentation, out_fname, dct, order=1, force_separate_z=None, order_z=0, verbose: bool = False):
    ...
    return seg_old_size.astype(np.float32)
