...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  ...
                  segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False, use_gaussian:bool = True):
    """
...
def predict_cases_fastest(model, list_of_lists, output_filenames, folds, num_threads_preprocessing,
                          ...
                          checkpoint_name="model_final_checkpoint", disable_postprocessing: bool = False,  use_gaussian:bool = True):
    assert len(list_of_lists) == len(output_filenames)
    if segs_from_prev_stage is not None: assert len(segs_from_prev_stage) == len(output_filenames)
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             disable_postprocessing=disable_postprocessing,use_gaussian=use_gaussian)
    elif mode == "fast":
        if overwrite_all_in_gpu is None:
            all_in_gpu = False
        else:
